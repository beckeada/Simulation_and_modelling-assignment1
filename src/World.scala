...
case class World(r_st: Int = 0) {
  ...
  var fel = mutable.PriorityQueue[Event]()(timeOrdering)
  ...
  def getStatistics:(Double,Double) = ((total_blocked.toDouble / total_calls),(total_dropped.toDouble / total_calls))
  ...
  def plan_call(): Unit = {
    if(warmUp && total_calls==1000)resetStats()

    val t:Double = time+RandomGenerator.getInterArrival
    fel.enqueue(Start(t))
  }

  def finished: Boolean = fel.isEmpty || total_calls >= 10000
